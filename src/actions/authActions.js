...
export const getUser = () => (dispatch, getState) => {
  ...
  console.log(state.auth.currUser);
  console.log(userId);
  console.log("authAction 38");
  initNotificationAndEventChannels(userId, dispatch, getState);
  })
  .catch((err) => {
    dispatch({
      type: AUTH_FAIL
...
export const login = ({ email, password }, history) => (dispatch, getState) => {
  ...
  axios
    ...
    .then((res) => {
      ...
      axios
      .get(`/api/online/${state.auth.currUser.id}`, headers, {withCredentials:true})
...
const initNotificationAndEventChannels = (userId, dispatch, getState) => {
