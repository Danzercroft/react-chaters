  ...
  return (
    <div className="userProfileContainer">
      ...
      <Modal isOpen={modal} toggle={toggle} className={className}>
        ...
        <span>{user.name}</span>
        {user.id !== currUser.id && <Button onClick={ () => addFriend(user.id)} color="success"> <i className="fas fa-user-plus"></i> Add Friend</Button>}
        <div>
        <h2>About</h2>
        { user.id === currUser.id && <Button onClick={toggleEdit} color="success"><i className="fas fa-edit"></i> { isEditOpen ? "Save Edit" : "Edit This" }</Button> }
        </div>
        <p>
